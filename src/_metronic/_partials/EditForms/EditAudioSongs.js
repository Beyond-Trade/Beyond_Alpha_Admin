...
export function EditAudioSong() {
  const history = useHistory();
  const params = useParams();
  console.log(params);
  const [songs, setSongs] = useState();
  const [fileErrorMessage, setFileErrorMessage] = useState("");
  const [invalidAudioExtension, setInvalidAudioExtension] = useState("");
  ...
  const [audiofileToUpload, setAudioFileToUpload] = useState(undefined);
  ...
  const handleAudioFileUpload = (e) => {
    if (e?.target?.files[0]) {
      ...
      setInvalidAudioExtension("");
      const res = splitByLastDot(e.target.files[0].name);
      if (res[1] === "mp3") {
        setInvalidAudioExtension("");
        console.log("file exxtension matched");
        setAudioFileToUpload(e.target.files[0]);
      } else {
        setInvalidAudioExtension("Please chose file with extension '.mp3' .");
        setAudioFileToUpload(undefined);
      }
    } else {
      setAudioFileToUpload(undefined);
    }
  };
  console.log(audiofileToUpload);
  ...
  const uploadDocument = async () => {
    if (audiofileToUpload && imagefileToUpload) {
      setIsSubmit(true);
      await uploadFile();
    } else if (!audiofileToUpload && !imagefileToUpload) {
      setIsSubmit(true);
      const allData = {
      ...
      firebase
        ...
        .then(() => {
          history.push("/audio_songs");
        });
      console.log(allData);
    } else {
      setSelectionError(
        "Please select the both Audio and Artwork file or unselect the selected one."
      );
      console.log("please select the both Audio and image file");
    }
  };
  useEffect(() => {
            ...
            <div className="card-body p-0">
              <form>
                ...
                <>
                      ...
                      <div className="col-sm-6">
                        <div
                          style={{
                            width: "50%",
                            height: "200px",
                            backgroundImage: `url(${songs.artwork})`,
                  ...
                  <div className="form-group">
                    <div className="row">
                      <div className="col-sm-6">
                        <label>Upload Audio Song To Update</label>
                        <input
                          type="file"
              ...
              <div className="float-right">
                <Button
                  variant="white"
                  type="submit"
                  onClick={() => history.push("/audio_songs")}
                >
                  Cancel
                </Button>
                {isSubmit ? (
